...
data = "/home/pi/stta/data/" + "TGZ_B_" + timestr+ ".txt"
f = open(data,  "w")
...
fig.canvas.manager.set_window_title("TGZ B")
...
(vol_line,) = ax.plot(x, y, label="TGZ B Gasvolumen", color="#00549F") #00549F is the RWTH blue color
ax.set_ylabel("TGZ B Gasvolumen in L", color="#00549F")
(rate_line,) = ax2.plot(x, y2, label="TGZ B Gasdurchflussrate", color="#CC071E", linestyle=" ", marker=".") #CC071E is the RWTH red (both colors as defined in the official RWTH guide)
ax2.set_ylabel("TGZ B Gasdurchflussrate in mL/h", color="#CC071E")
# plt.ylim(0, 150) commented out because it overrides dynamic axes scaling
plt.title("TGZ B Gasvolumen: " + str(gas_volume) + "L", fontsize=25) # To display 0ml initially. Will be updated in an event-driven manner (see on_trigger)
...
try:
    while True:
        ...
        plt.title("TGZ B CO2-Volumen: " + str(gas_volume) + " L  " + "Volumenstrom CO2: " + str(r) + " mL/h", fontsize=10)#Diagrammtitel
