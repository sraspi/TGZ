...
data = "/home/pi/stta/data/" + "TGZ_A_" + timestr+ ".txt"
f = open(data,  "w")
...
fig.canvas.manager.set_window_title("TGZ A")
...
(vol_line,) = ax.plot(x, y, label="TGZ A Gasvolumen", color="#00549F") #00549F is the RWTH blue color
ax.set_ylabel("TGZ A Gasvolumen in L", color="#00549F")
(rate_line,) = ax2.plot(x, y2, label="TGZ A Gasdurchflussrate", color="#CC071E", linestyle=" ", marker=".") #CC071E is the RWTH red (both colors as defined in the official RWTH guide)
ax2.set_ylabel("TGZ A Gasdurchflussrate in mL/h", color="#CC071E")
# plt.ylim(0, 150) commented out because it overrides dynamic axes scaling
plt.title("TGZ A Gasvolumen: " + str(gas_volume) + "L", fontsize=25) # To display 0ml initially. Will be updated in an event-driven manner (see on_trigger)
...
try:
    while True:
        ...
        plt.title("TGZ A CO2-Volumen: " + str(gas_volume) + " L  " + "Volumenstrom CO2: " + str(r) + " mL/h", fontsize=10)#Diagrammtitel
